...
$ProjectRoot = Join-Path $LauncherRoot "MovieWatchlistTracker"
...
$StartBat = Join-Path $LauncherRoot "START_HERE.bat"
$AppHost = "127.0.0.1"
$PreferredPort = 5093
...
function Show-ManualCommands {
    ...
    Write-Host "dotnet clean `"$ProjectFile`""
    Write-Host "dotnet restore `"$ProjectFile`""
    Write-Host "dotnet restore `"$TestProjectFile`""
    Write-Host "dotnet build `"$ProjectFile`""
    Write-Host "dotnet run --no-build --project `"$ProjectFile`" --urls http://$AppHost`:$PreferredPort"
    Write-Host ""
