...
$CoverRestoreScript = Join-Path $ProjectRoot "tools\restore-cover-posters.ps1"
...
function Restore-CoverPosters {
    if (-not (Test-Path $CoverRestoreScript)) {
        Write-Host "Cover poster restore script was not found: $CoverRestoreScript" -ForegroundColor Yellow
        return
    }

    Write-Host "Checking cover poster files..."
    & $CoverRestoreScript
}
